...
export default function Command() {
  ...
  const [enrichedArticles, setEnrichedArticles] = useState<Map<string, any>>(
    new Map(),
  );
  ...
  async function loadArticleDetails(article: Article) {
    ...
    // Don't load if we already have the data
    if (enrichedArticles.has(articleId)) {
      return;
    }
    ...
    try {
      ...
      // Update the enriched articles map
  ...
  return (
    <List
      ...
      searchBarPlaceholder="Pesquisar notícias do Publico..."
      isShowingDetail
      throttle
    ...
    >
      {searchText.trim() === "" ? (
        <List.EmptyView
          ...
          title="Pesquisar notícias do Publico"
          description="Comece a digitar para pesquisar artigos."
        />
      ) : articles.length === 0 && !isLoading ? (
        <List.EmptyView
          icon={Icon.XmarkCircle}
          title="Nenhum artigo encontrado"
          description={`Sem resultados para "${searchText}". Tente outro termo.`}
        />
      ) : (
        articles.map((article, index) => {
          ...
          // Extract article ID
          const articleId = articleUrl ? extractArticleId(articleUrl) : null;
          ...
          // Check if this is the article we're currently loading details for
          const isLoading = articleId === selectedArticleId && isLoadingDetails;
